...
disk_size_bytes="${ARGOSFS_QEMU_CHAOS_DISK_SIZE_BYTES:-268435456}"
...
printf 'post-recovery write\n' >/root/argosfs-chaos-after-recovery/payload.txt
sync
...
run_phase1_until_kill_marker() {
  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    ...
    if [ "$unplugged" -eq 0 ] && grep -Fq ARGOSFS_WAIT_CHAOS_UNPLUG "$log1" 2>/dev/null; then
      qemu_device_del chaos 1
      unplugged=1
    fi
    if grep -Fq ARGOSFS_CHAOS_READY_FOR_HARD_KILL "$log1" 2>/dev/null; then result=0; break; fi
    if ! kill -0 "$qemu_pid" 2>/dev/null; then result=3; break; fi
    sleep 1
  done
  if [ "$result" -eq 0 ] && kill -0 "$qemu_pid" 2>/dev/null; then argosfs_qemu_kill_tree "$qemu_pid"; fi
