...
mod tests {
    ...
    fn superblock_constructor_and_label_validation_cover_conversions_and_mismatches() {
        ...
        let sb = superblock_for_device(pool, 0, "disk-0000", 1, 0, 4096, MIN_DEVICE_BYTES, "pool")
            .unwrap();
    ...
    fn loop_pool_lifecycle_scans_inspects_audits_and_uses_backup_superblock() {
        ...
        let opened =
            open_pool(BackendKind::LoopBlock, std::slice::from_ref(&image), false).unwrap();
        ...
        let (_, source) =
            read_superblock_with_backup(&backend, "disk-0000", MIN_DEVICE_BYTES).unwrap();
        assert_eq!(source, "backup");
        let (backup_sb, backup_label) =
            inspect_device(BackendKind::LoopBlock, image.clone()).unwrap();
